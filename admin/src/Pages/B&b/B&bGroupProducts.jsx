...
const API_URL_PRODUCTS = 'http://localhost:8000/api/bnb-group-product';
const API_URL_ALL_PRODUCTS = 'http://localhost:8000/api/productsroute';
const API_URL_GROUP = 'http://localhost:8000/api/bnb-group';

const BandBGroupProducts = () => {
  const { id } = useParams();
...
    try {
      // Fetch the details of the specific B&B Group
      const groupRes = await axios.get(`${API_URL_GROUP}/${id}`);
      setGroup(groupRes.data.bandbGroup);

      // Fetch the products associated with this B&B Group
...
          data: { product_id: productId, bnb_group_id: id } // Updated column name
        });
...
        bnb_group_id: id, // Updated column name
      });
...
        onClick={() => navigate('/bnb-groups')} // Updated navigation path
      >
        ← Back to the Groups
      </button>

      <h1 className="text-3xl font-bold mb-2">Manage Products for the B&B Group: {group.name}</h1>
...
export default BandBGroupProducts;