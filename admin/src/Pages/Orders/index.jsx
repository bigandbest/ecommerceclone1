...
const AdminOrders = () => {
  ...
  const fetchOrders = async () => {
    const res = await axios.get("https://ecommerceclone1.onrender.com/api/order/all");
    setOrders(res.data.orders || []);
  };

  const deleteOrder = async (order_id) => {
    if (!window.confirm("Are you sure you want to delete this order?")) return;
    await axios.delete(`https://ecommerceclone1.onrender.com/api/order/delete/${order_id}`);
    fetchOrders(); // Refresh list
  };

  const updateOrder = async (id, status, adminnotes) => {
    await axios.put(`https://ecommerceclone1.onrender.com/api/order/status/${id}`, { status, adminnotes });
    fetchOrders(); // Refresh
  };
...
const OrderItems = ({ orderId, onUpdate, status, adminnotes }) => {
  ...
  useEffect(() => {
    axios.get(`https://ecommerceclone1.onrender.com/api/orderItems/order/${orderId}`).then((res) => {
